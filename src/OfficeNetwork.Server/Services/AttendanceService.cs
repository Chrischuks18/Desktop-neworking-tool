...
public sealed class AttendanceService
{
    ...
    {
        using var c=Open();using var cmd=c.CreateCommand();cmd.CommandText="SELECT Id,WorkDate,ClockIn,Status FROM Attendance WHERE UserId=$u AND ClockOut IS NULL ORDER BY ClockIn DESC LIMIT 1";cmd.Parameters.AddWithValue("$u",u.Id.ToString());
    ...
    {
        using var c=Open();using var cmd=c.CreateCommand();cmd.CommandText="SELECT Id,WorkDate,ClockIn,ClockOut,Status FROM Attendance WHERE UserId=$u ORDER BY ClockIn DESC LIMIT 1";cmd.Parameters.AddWithValue("$u",u.Id.ToString());using var r=cmd.ExecuteReader();if(!r.Read())return null;
