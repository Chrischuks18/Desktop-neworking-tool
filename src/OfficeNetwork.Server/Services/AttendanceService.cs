...
public sealed class AttendanceService
{
    ...
    public AttendanceRecord ClockIn(OfficeUser u)
    {
        ...
        using var c=Open(); using(var find=c.CreateCommand()){find.CommandText="SELECT Id FROM Attendance WHERE UserId=$u AND WorkDate=$d LIMIT 1";find.Parameters.AddWithValue("$u",u.UserId.ToString());find.Parameters.AddWithValue("$d",day);if(find.ExecuteScalar()!=null)throw new InvalidOperationException("You have already clocked in today.");}
